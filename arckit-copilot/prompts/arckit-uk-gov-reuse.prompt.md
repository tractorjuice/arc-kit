...
**`arckit-uk-gov-reuse-reader`** subagent (one call per capability) via
...
**`arckit-uk-gov-reuse-writer`** subagent to render the final artefact.
...
(which runs in the main thread) rather than in an `arckit-uk-gov-reuse`
...
2. If `${input:topic:Enter project name or topic}` contains a bare project number (e.g. `001`) or name fragment, glob `projects/{NUMBER}-*/` or `projects/*-*{NAME}*/` and use the unique match. If multiple match, ask the user to disambiguate before proceeding — do not default to "most recent".
...
Use enum values from `.arckit/schemas/gov-reuse-handoff.schema.json` so the profile aligns with the reader's evidence allowlist.
...
- If matched: rubric = `.arckit/schemas/scoring-rubrics/gov-reuse-uk-gov.yaml`
- Otherwise: rubric = `.arckit/schemas/scoring-rubrics/gov-reuse-generic.yaml`
...
Ensure `.arckit/scripts/validate-handoff.mjs` exists via `Read`. If missing, stop and tell the user the plugin install is incomplete.
...
2. Dispatch the reader using the `Agent` tool with `subagent_type: "arckit-uk-gov-reuse-reader"` and the input JSON as the prompt.
...
3. The reader's final-message string is a JSON payload. Write it to a tempfile via Bash, run the validator, and capture the result:
   ...
   node ".arckit/scripts/validate-handoff.mjs" \
        ".arckit/schemas/gov-reuse-handoff.schema.json" \
        "$TMPFILE"
   echo "exit=$?"
   rm -f "$TMPFILE"
   ```
...
- **Templates** — `.arckit/templates/uk-gov-reuse-template.md` · `.arckit/templates/tech-note-template.md` (read by writer)
- **Schemas** — `.arckit/schemas/gov-reuse-handoff.schema.json` · `.arckit/schemas/scoring-rubrics/gov-reuse-{generic,uk-gov}.yaml`
...
- **Subagents dispatched** — `arckit-uk-gov-reuse-reader` (per capability) · `arckit-uk-gov-reuse-writer` (final render)
