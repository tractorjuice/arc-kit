...
**This command delegates to the `arckit-uk-gov-code-search` agent** which runs as an autonomous subprocess. The agent makes multiple WebSearch and WebFetch calls against the govreposcrape API and individual GitHub repositories to gather search results, README content, and code patterns — running in its own context window to avoid polluting the main conversation with large repository content.
...
2. **Launch the agent**: Launch the **arckit-uk-gov-code-search** agent in `acceptEdits` mode with the following prompt:

   ```text
   Search UK government repositories for: {${input:topic:Enter project name or topic}}
...
2. **Read the template** (with user override support):
   - **First**, check if `.arckit/templates/uk-gov-code-search-template.md` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `.arckit/templates/uk-gov-code-search-template.md` (default)
...
- Next steps (`/arckit-uk:uk-gov-reuse`, `/arckit:research`)

## Integration with Other Commands

- **Input**: Uses requirements document (`ARC-*-REQ-*.md`) — OPTIONAL
- **Output**: Feeds into `/arckit-uk:uk-gov-reuse` (deep reuse assessment of interesting finds)
