...
**This command delegates to the `arckit-uk-gov-landscape` agent** which runs as an autonomous subprocess. The agent makes multiple WebSearch and WebFetch calls against the govreposcrape API and individual GitHub repositories to gather organisational profiles, technology inventories, and activity signals — running in its own context window to avoid polluting the main conversation with large repository content.
...
2. **Launch the agent**: Launch the **arckit-uk-gov-landscape** agent in `acceptEdits` mode with the following prompt:

   ```text
   Map the UK government code landscape for the project in projects/{project-dir}/.

   Domain focus: {${input:topic:Enter project name or topic}}
...
2. **Read the template** (with user override support):
   - **First**, check if `.arckit/templates/uk-gov-landscape-template.md` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `.arckit/templates/uk-gov-landscape-template.md` (default)
...
- **Output**: Feeds into `/arckit-uk:uk-gov-reuse` (assess specific repositories for reuse)
