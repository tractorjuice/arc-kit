...
1. Read `${CLAUDE_PLUGIN_ROOT}/skills/mermaid-syntax/references/c4-layout-science.md` for research-backed graph drawing guidance — Sugiyama algorithm, tier-based declaration ordering, edge crossing targets, C4 colour standards, and prompt antipatterns.
2. Read the type-specific Mermaid syntax reference:
   ...
   - **Data Flow with ER content**: Also read `${CLAUDE_PLUGIN_ROOT}/skills/mermaid-syntax/references/entityRelationshipDiagram.md`

**If PlantUML format selected:**

1. Read `${CLAUDE_PLUGIN_ROOT}/skills/plantuml-syntax/references/c4-plantuml.md` for C4-PlantUML element syntax, directional relationships, layout constraints, and **layout conflict rules** (critical for preventing `Rel_Down`/`Lay_Right` contradictions).
2. For Sequence diagrams: also read `${CLAUDE_PLUGIN_ROOT}/skills/plantuml-syntax/references/sequence-diagrams.md`
