...
handoffs:
  - command: service-assessment
    description: The readiness gap register feeds the broader service-assessment evidence pack.
  - command: roadmap
    description: Remediation actions for FedRAMP gaps drop into the architecture roadmap timeline.
  - command: risk
    description: Open gaps and POA&M items become entries in the project risk register.
---
...
Surfaced gaps feed the broader `/arckit-uk:uk-service-assessment` evidence pack and remediation timelines drop into `/arckit:roadmap`. Each open gap should be tracked in `/arckit:risk` until closed. Once gaps are remediated, re-run this command to re-baseline before engaging a 3PAO.
