...
`/arckit-uk:uk-gov-reuse` searches 24,500+ UK government open-source repositories to discover existing implementations that can be reused, forked, or referenced before building from scratch.
...
```bash
/arckit-uk:uk-gov-reuse <capability or domain>
```
...
| Feature | `/arckit-uk:uk-gov-reuse` | `/arckit.research` |
|---------|--------------------|--------------------|
...
**Workflow**: Run `/arckit-uk:uk-gov-reuse` first, then `/arckit.research` to compare reuse candidates against commercial alternatives.
