...
description: |
  ...
  user: "/arckit-uk:uk-gov-landscape Map the government landscape for health data integration"
  ...
  user: "/arckit-uk:uk-gov-landscape Survey government use of event-driven architecture"
...
- If not found, read `${CLAUDE_PLUGIN_ROOT}/templates/uk-gov-landscape-template.md` (default)
...
This is a landscape-level signal, **not** a per-repo security audit: report exposure breadth and the highest-impact advisories, and recommend `/arckit-uk:uk-secure` or `/arckit:risk` for any repo the project intends to adopt. If `vulnerability_exposure` returns no data for a scope (e.g. an org with no indexed SBOMs), note it as a coverage gap rather than "no vulnerabilities".
...
```text
**Generated by**: ArcKit `/arckit-uk:uk-gov-landscape` agent
**Generated on**: {DATE}
...
- Key gaps identified
- Next steps (`/arckit-uk:uk-gov-reuse`, `/arckit:framework`, `/arckit:wardley`)
...
- **Templates** — `${CLAUDE_PLUGIN_ROOT}/templates/uk-gov-landscape-template.md` (override at `.arckit/templates-custom/gov-landscape-template.md`)
...
- **Related commands** — `/arckit-uk:uk-gov-reuse` (capability-driven reuse) · `/arckit-uk:uk-gov-code-search` (focused queries)
