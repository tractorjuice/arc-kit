---
name: arckit-uk-gov-code-search
maxTurns: 40
tools:
  - Read
  - Glob
...
description: |
  ...
  user: "/arckit-uk:uk-gov-code-search How did government teams implement FHIR patient data integration?"
  ...
  user: "/arckit-uk:uk-gov-code-search GOV.UK Design System accessible form components"
...
Extract the search query from the user's arguments. The query is what follows the `/arckit-uk:uk-gov-code-search` command invocation. Preserve the user's intent exactly — do not summarise or rephrase their query at this stage.
...
- If not found, read `${CLAUDE_PLUGIN_ROOT}/templates/uk-gov-code-search-template.md` (default)
...
```text
**Generated by**: ArcKit `/arckit-uk:uk-gov-code-search` agent
**Generated on**: {DATE}
...
- **Templates** — `${CLAUDE_PLUGIN_ROOT}/templates/uk-gov-code-search-template.md` (override at `.arckit/templates-custom/gov-code-search-template.md`)
...
- **Related commands** — `/arckit-uk:uk-gov-reuse` (capability-driven reuse) · `/arckit-uk:uk-gov-landscape` (domain landscape)
