...
description: 'Use this agent when the user wants to discover reusable UK government
  ...
  user: "/arckit-uk:uk-gov-reuse Check for existing government code for appointment booking"
...
- If not found, read `${CLAUDE_PLUGIN_ROOT}/templates/uk-gov-reuse-template.md` (default)
...
```text
**Generated by**: ArcKit `/arckit-uk:uk-gov-reuse` agent
**Generated on**: {DATE}
...
- **Templates** — `${CLAUDE_PLUGIN_ROOT}/templates/uk-gov-reuse-template.md` (override at `.arckit/templates-custom/gov-reuse-template.md`)
...
- **Related commands** — `/arckit:requirements` (input) · `/arckit:research` (build-vs-buy comparison) · `/arckit-uk:uk-gov-code-search` · `/arckit-uk:uk-gov-landscape`
