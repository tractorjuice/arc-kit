...
1. **Loading the template** (with user override support):
   - **First**, check if `.arckit/templates/uk-secure-template.md` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/uk-secure-template.md` (default)
...
15. **Save the document**:
    ...
    Before writing the file, read `${CLAUDE_PLUGIN_ROOT}/references/quality-checklist.md` and verify all **Common Checks** plus the **SECD** per-type checks pass. Fix any failures before proceeding.
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/arckit-uk:uk-secure` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/arckit-uk:uk-secure` command
...
|---------|------|--------|---------|-------------|---------------|
| 1.0 | 2025-10-29 | ArcKit AI | Initial creation from `/arckit-uk:uk-secure` command | [PENDING] | [PENDING] |
```
