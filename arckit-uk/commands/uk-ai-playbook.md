...
3. **Read existing artifacts from the project context:**
   ...
   **Read the template** (with user override support):
   - **First**, check if `.arckit/templates/uk-ai-playbook-template.md` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/uk-ai-playbook-template.md` (default)
