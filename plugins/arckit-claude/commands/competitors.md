...
handoffs:
  - command: research
    description: Feed the competitive set into build-vs-buy analysis
  - command: score
...
using the explicit scope in `$ARGUMENTS` and say so in your first message
...
Ensure `${CLAUDE_PLUGIN_ROOT}/scripts/validate-handoff.mjs` exists via
