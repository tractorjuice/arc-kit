...
1a. **Tidy OWM labels** — *only if `--tidy-owm` was passed in `$ARGUMENTS`*. Rewrite the
   ...
   ```bash
   node ${CLAUDE_PLUGIN_ROOT}/hooks/owm-tidy.mjs /tmp/arckit-wardley.owm
   ```
...
2. Run the converter and capture the output:
   ...
   ```bash
   node ${CLAUDE_PLUGIN_ROOT}/scripts/owm-to-mermaid.mjs /tmp/arckit-wardley.owm
   ```
...
default; skip it only if `--no-html` was passed in `$ARGUMENTS`.

```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/owm-to-html.mjs /tmp/arckit-wardley.owm \
