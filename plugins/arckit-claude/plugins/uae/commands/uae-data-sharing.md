...
2. Read the template:
   ...
   - **Fallback**, `${CLAUDE_PLUGIN_ROOT}/templates/uae-data-sharing-template.md`
3. Use `scripts/bash/create-project.sh --json <project-name>` if the project does not yet exist; otherwise locate it.
4. Use `scripts/bash/generate-document-id.sh <PROJECT_ID> DSHR --filename` for the artefact filename.
...
- Every share involving personal data MUST cite a PDPL lawful basis. Where `/arckit-uae:uae-pdpl` has not yet been run, recommend it as a follow-up before sign-off.
