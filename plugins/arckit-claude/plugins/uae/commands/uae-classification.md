...
4. Use `scripts/bash/generate-document-id.sh <PROJECT_ID> CLAS --filename` for the artefact filename.
...
- Confidential, Secret and Top Secret datasets carry residency obligations under the National Cloud Security Policy v2 — flag these for `/arckit-uae:uae-cloud-residency`.
- Personal data datasets must be mapped to a PDPL lawful basis — if `/arckit-uae:uae-pdpl` has not yet been run, recommend it as a follow-up.
