...
doc-type: OAGOV
argument-hint: "<product or programme name, e.g. 'Observability Stack', 'enterprise AI platform'>"
effort: medium
handoffs:

  - command: oaa-adm-lite
    description: Re-plan sprint cadence based on governance findings
...
- **`compliance-evidence.yaml`** — sprint compliance evidence, always current (shared with `/arckit-oaa:agile-security`)

- **`vision.yaml`** — governance constraints from architecture vision (inherited from `/arckit-oaa:oaa-adm-lite`)
