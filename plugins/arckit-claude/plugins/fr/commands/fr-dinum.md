...
handoffs:
  - command: fr-rgpd
    description: Assess CNIL-specific GDPR obligations after establishing DINUM compliance baseline
    condition: "Service processes personal data of French residents"
  - command: fr-secnumcloud
    description: Assess SecNumCloud requirements if cloud hosting is involved
    condition: "Cloud hosting required under doctrine cloud de l'État"
  - command: secure
    description: Generate Secure by Design assessment aligned with RGS findings
---
...
- **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/fr-dinum-template.md`
- **Then read** `${CLAUDE_PLUGIN_ROOT}/templates/_partials/RENDERING.md` and resolve the `<!-- DOC-CONTROL-HEADER -->` marker in the template before writing. `DINUM` carries the France regime, which hard-routes to `_partials/document-control-fr.md` (Non protégé / Diffusion Restreinte / Secret / Très Secret) whatever the operator's user config says.
...
Before writing the file, read `${CLAUDE_PLUGIN_ROOT}/references/quality-checklist.md` and verify all **Common Checks** plus **DINUM** per-type checks pass.
...
Next steps:
1. {If personal data: Run /arckit-fr:fr-rgpd for CNIL compliance}
2. {If cloud: Run /arckit-fr:fr-secnumcloud for cloud doctrine alignment}
...
```text
/arckit-fr:fr-dinum Assess DINUM standards compliance for a citizen-facing tax declaration portal operated by a French ministry, handling personal and financial data, targeting full RGAA compliance and SecNumCloud hosting, with FranceConnect integration

/arckit-fr:fr-dinum DINUM compliance for 001 — regional government digital service, 300 agents, partial cloud migration to OVHcloud

/arckit-fr:fr-dinum Assess digital standards for a French local authority (mairie) citizen portal, under 250 agents, RGAA recommended not mandatory
```
