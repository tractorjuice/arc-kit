...
handoffs:
  - command: dpia
    description: Run a full Data Protection Impact Assessment if CNIL screening flags high risk
    condition: "2+ CNIL DPIA criteria triggered"
  - command: fr-secnumcloud
    description: Assess SecNumCloud requirements for health data hosting
    condition: "Health data (données de santé) processed — HDS hosting required"
  - command: fr-marche-public
    description: Include GDPR/DPA obligations in procurement documentation
    condition: "Procurement involves data processors"
---
...
- **RGPD** (EU RGPD Assessment) — Extract: legal basis mapping, DPIA screening results, DPO determination, international transfer analysis
  - If missing: warn that `/arckit-fr:fr-rgpd` should be run after `/arckit-eu:eu-rgpd` for best results. Proceed with available data.
...
- **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/fr-rgpd-template.md`
- **Then read** `${CLAUDE_PLUGIN_ROOT}/templates/_partials/RENDERING.md` and resolve the `<!-- DOC-CONTROL-HEADER -->` marker in the template before writing. `CNIL` carries the France regime, which hard-routes to `_partials/document-control-fr.md` (Non protégé / Diffusion Restreinte / Secret / Très Secret) whatever the operator's user config says.
...
Before writing the file, read `${CLAUDE_PLUGIN_ROOT}/references/quality-checklist.md` and verify all **Common Checks** plus **CNIL** per-type checks pass.
...
Next steps:
1. {If health data: Run /arckit-fr:fr-secnumcloud for HDS-compliant hosting}
2. {If DPIA required: Run /arckit:dpia}
3. {If procurement: Run /arckit-fr:fr-marche-public for DPA clauses}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

## Important Notes

- **Run after eu-rgpd**: This command adds the French layer on top of the EU GDPR baseline. For best results, run `/arckit-eu:eu-rgpd` first, then this command.
...
> **Note for reviewers**: This command covers France-specific GDPR obligations layered on top of the baseline EU GDPR (covered by `/arckit-eu:eu-rgpd`). Key French specifics: the age of digital consent is **15** (not the GDPR default of 16), HDS (Hébergement de Données de Santé) is a mandatory French certification for any cloud provider hosting health data, and the CNIL has issued specific guidance on analytics tools — notably ruling that Google Analytics transfers personal data to the US unlawfully (2022). The CNIL is the French Data Protection Authority (DPA), member of the EDPB.
...
```text
/arckit-fr:fr-rgpd Assess CNIL compliance for a French regional hospital group deploying a patient portal, processing données de santé, with third-party analytics and a mobile app targeting both adults and teenagers

/arckit-fr:fr-rgpd CNIL layer for 001 — e-commerce platform with Google Analytics, loyalty profiling, EU-US transfers

/arckit-fr:fr-rgpd French GDPR layer for a ministry HR system handling agent personal data, DPO mandatory, no health data
```
