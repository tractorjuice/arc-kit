...
handoffs:
  - command: fr-ebios
    description: Use ANSSI hygiene gap findings as Workshop 1 security baseline in the EBIOS risk analysis
    condition: "ANSSI assessment reveals significant gaps that should inform a formal risk analysis"
  - command: fr-secnumcloud
    description: Assess cloud provider qualification against SecNumCloud when cloud security gaps are identified
    condition: "Cloud security recommendations show gaps around provider qualification or extraterritorial risk"
  - command: fr-pssi
    description: Translate ANSSI compliance findings into formal PSSI security objectives and organisational measures
    condition: "Organisation requires a formal security policy document"
  - command: secure
    description: Implement the technical security measures identified in the ANSSI gap analysis
    condition: "ANSSI hygiene gaps require implementation in the codebase or infrastructure"
---
...
- **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/fr-anssi-template.md`
- **Then read** `${CLAUDE_PLUGIN_ROOT}/templates/_partials/RENDERING.md` and resolve the `<!-- DOC-CONTROL-HEADER -->` marker in the template before writing. `ANSSI` carries the France regime, which hard-routes to `_partials/document-control-fr.md` (Non protégé / Diffusion Restreinte / Secret / Très Secret) whatever the operator's user config says.
...
Before writing the file, read `${CLAUDE_PLUGIN_ROOT}/references/quality-checklist.md` and verify all **Common Checks** plus **ANSSI** per-type checks pass.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Next steps:
1. {If OIV/OSE: Run /arckit-fr:fr-ebios — ANSSI findings feed Workshop 1 baseline}
2. {If cloud gaps: Run /arckit-fr:fr-secnumcloud for provider qualification assessment}
3. Run /arckit-fr:fr-pssi to formalise security objectives in a PSSI document
...
- **OIV / OSE obligations**: For OIV systems (SIIV), the hygiene measures are a baseline minimum. The sectoral arrêté sectoriel may impose additional measures. For OSE under NIS2, Article 21 measures overlap significantly — run `/arckit-eu:eu-nis2` to avoid duplication.
...
```text
/arckit-fr:fr-anssi Assess ANSSI hygiene compliance for a French regional prefecture information system — on-premise Windows/Active Directory environment, 300 users, no cloud services

/arckit-fr:fr-anssi ANSSI security posture for 001 — hybrid cloud ministry portal, hosted on OVHcloud, handling citizen personal data, NIS2 OSE designation

/arckit-fr:fr-anssi ANSSI hygiene assessment for a French private company (OIV énergie sector), SCADA-adjacent IS, mixed cloud and on-premise, 50 IT staff
```
