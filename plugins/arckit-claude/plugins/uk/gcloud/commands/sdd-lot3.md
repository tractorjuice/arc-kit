---
description: Generate the Service Definition Document for Lot 3 (Cloud Support)
effort: max
handoffs:
...
> planning aid for a UK G-Cloud / Digital Marketplace **Lot 3 (Cloud Support)** service offering; it
...
> staff clearances, SFIA mappings, day rates) before publishing a live framework listing.

You are helping a cloud service supplier generate a complete **Service Definition Document (SDD)** for
**Lot 3 (Cloud Support — professional services)** on the G-Cloud Digital Marketplace.
...
- Service design (this project): use the **Read tool** on the resolved project's
  `ARC-{PROJECT_ID}-SVCD-v*.md` (the SVCD doc written by `/arckit-uk-gcloud:service-design`).
...
- **Supplier website / services page** — service offerings and capabilities, team credentials and
  certifications, case studies and experience, security-clearance availability, methodology approach.
- **Existing Digital Marketplace listing** —
  `site:applytosupply.digitalmarketplace.service.gov.uk "[supplier name]" "Lot 3"` to see current
  day-rate positioning, how skills are described, and service categories offered.
- **SFIA reference** — `SFIA 8 [skill name] level responsibilities` to verify skill definitions and
  level descriptors.
...
Use the **Read tool** to read the Lot 3 template:

- `${CLAUDE_PLUGIN_ROOT}/templates/sdd-lot3-template.md`
...
Lot 3 (Cloud Support) covers **professional services** rather than hosted services. Generate answers
for **all** questions in the template using the service design and supplier profile to keep answers
accurate and **consistent**.

**CRITICAL:** The SDD must answer every mandatory question — missing answers cause rejection.

#### Lot 3 specific categories

**Service type:** cloud migration; implementation; managed services; consultancy; training;
architecture design; optimisation; security services.

**Delivery model:** remote; on-site; hybrid; fixed price vs time & materials.

**Skills & expertise:** SFIA skills and levels; cloud platform expertise (AWS, Azure, GCP, etc.);
technology certifications; industry experience.

**Team composition:** typical team structure; minimum / maximum team sizes; named resources vs
resource pool; subcontracting approach.

**Methodologies:** project management (Agile, Waterfall, Hybrid); ITIL alignment; DevOps practices;
Security by Design.

**Quality assurance:** QA processes; testing approach; acceptance criteria; knowledge transfer.

**Pricing structure:** day rates by role / SFIA level; fixed-price options; volume discounts; travel
and expenses policy.

**Security:** staff security clearances (BPSS, SC, DV); background checking; security training; data
handling during engagement.

**Governance:** reporting approach; escalation procedures; change control; risk management.

#### SFIA skills mapping

Map the service capabilities to the **SFIA 8** framework, giving each skill a level (1–7) — for
example `ARCH` (Solution Architecture) at level 6, `PROG` (Programming / Software Development) at
level 5. Accurate SFIA mapping matters for buyer searching. Use WebSearch
(`SFIA 8 [skill name] level responsibilities`) to verify skill codes and level descriptors if
unsure.
...
`**G-Cloud Lot**` line — record this service as **Lot 3 (Cloud Support)**. Respect marketplace limits
(≤ 100 chars per feature/benefit, ≤ 50 words for the service summary). Leave genuinely-unknown fields
as `[PENDING]` rather than inventing values. Where a fact came from a fetched source, attach the
appropriate inline citation marker (see Step 3).
...
### Service Summary
- Type: [Migration / Implementation / Managed / Consultancy]
...
- SC Cleared: [X] staff
- DV Cleared: [X] staff
- AWS Certified: [X] staff
- Azure Certified: [X] staff

### SFIA Skills
| Skill | Level | Description |
|-------|-------|-------------|
| ARCH | 6 | Solution Architecture |
| PROG | 5 | Programming / Software Dev |

### Day Rates (indicative)
| Role | SFIA Level | Day Rate |
|------|------------|----------|
| Consultant | 4 | £XXX |
| Senior Consultant | 5 | £XXX |
| Principal | 6 | £XXX |

### Next Steps
- Produce the pricing document (detailed rate card) for this service: `/arckit-uk-gcloud:pricing`
- Generate NCSC Cloud Security Principles assertions / clearances: `/arckit-uk-gcloud:security`

### Items Requiring Attention
- [Any [PENDING] fields the user must supply]
- [If the service design or supplier profile was missing: run `/arckit-uk-gcloud:service-design` /
  `/arckit-uk-gcloud:supplier-profile` to enrich this SDD]
```

## Important Notes

- Lot 3 is people-based — security clearances are critical and must be evidenceable.
- SFIA skill mapping matters for buyer searching.
- Day rates will be published — ensure competitiveness; subcontracting must be declared.
- Insurance requirements may be higher for consultancy; data-centre / clearance facts must match the
  supplier profile.
