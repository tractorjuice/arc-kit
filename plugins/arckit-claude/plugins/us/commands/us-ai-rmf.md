...
handoffs:
  - command: us-ai-impact
    description: Translate AI RMF findings into the M-24-10 rights-impacting / safety-impacting determination and the M-25-21 acquisition controls.
  - command: us-privacy-pia
    description: AI systems trained on or inferencing over PII require an E-Gov Act §208 PIA; the AI RMF data inventory seeds the PIA.
  - command: risk
    description: Residual AI risks (confabulation, bias, security, value-chain) flow into the project risk register.
  - command: adr
    description: Model architecture, hosting, data-governance, and human-oversight decisions made during the RMF process warrant ADRs.
---
...
The RMF findings drive `/arckit-us:us-ai-impact` — the M-24-10 rights/safety-impacting determination and M-25-21 acquisition controls inherit the RMF risk register. PII-handling AI systems require `/arckit-us:us-privacy-pia`. Residual risks flow into `/arckit:risk`. Significant model, hosting, data-governance, and human-oversight decisions captured during the RMF process should be recorded as ADRs via `/arckit:adr`.
