...
handoffs:
  - command: us-fedramp-readiness
    description: The SSP is the primary input to the 3PAO Readiness Assessment Report; gaps surfaced during SSP authoring populate the RAR gap register.
  - command: us-zero-trust
    description: SSP control implementations seed the CISA Zero Trust Maturity scoring (Identity, Devices, Networks, Apps & Workloads, Data pillars).
  - command: us-icam
    description: The Types of Users section and IA-family control implementations connect to the ICAM architecture.
---
...
Once the SSP is in draft, run `/arckit-us:us-fedramp-readiness` to produce the 3PAO-style Readiness Assessment Report (RAR) capturing gaps against the SSP control claims. The SSP control implementations also drive the Zero Trust scoring (`/arckit-us:us-zero-trust`) and the ICAM architecture (`/arckit-us:us-icam`). Boundary, interconnection, and Types-of-Users content must be kept consistent with the architecture diagrams under `projects/<id>/diagrams/`.
