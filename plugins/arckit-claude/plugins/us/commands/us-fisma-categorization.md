...
handoffs:
  - command: us-nist-800-53
    description: The FIPS 199 high-water mark drives the NIST SP 800-53 Rev 5 baseline (Low / Moderate / High) for control tailoring.
  - command: us-privacy-pia
    description: Information types containing PII trigger an E-Government Act §208 PIA; the FIPS 199 inventory seeds the PIA personal-information register.
  - command: risk
    description: Categorization rationale and any ambiguous information-type mappings feed the project risk register.
---
...
The categorization output directly feeds `/arckit-us:us-nist-800-53` (the water-mark selects the Low / Moderate / High baseline). If any information type contains PII, run `/arckit-us:us-privacy-pia` next to discharge the E-Government Act §208 PIA obligation. Any ambiguous or upgraded categorizations should be logged into the project risk register via `/arckit:risk` for AO visibility.
