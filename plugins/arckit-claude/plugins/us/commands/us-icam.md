...
handoffs:
  - command: us-zero-trust
    description: ICAM is the foundation of the Zero Trust Identity pillar; IAL/AAL/FAL selections directly score ZTMM Identity functions.
  - command: us-privacy-pia
    description: Identity proofing collects and processes PII (especially IAL2/IAL3); the ICAM data flows feed the PIA personal-information inventory.
  - command: adr
    description: Identity provider selection (PIV vs login.gov vs agency-specific) and federation pattern decisions warrant ADRs.
---
...
ICAM is the foundation of the Zero Trust Identity pillar — feed IAL/AAL/FAL outputs into `/arckit-us:us-zero-trust` scoring. The proofing data flows (especially at IAL2/IAL3) collect significant PII, so trigger `/arckit-us:us-privacy-pia`. Identity-provider selection and federation pattern decisions should be captured as ADRs via `/arckit:adr`.
