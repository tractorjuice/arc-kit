...
doc-type: NLEXIT
argument-hint: "<project ID or cloud service description, e.g. '001', 'email and document collaboration service currently hosted in public cloud'>"
effort: medium
handoffs:
  - command: nl-cloud
    description: Confirm the materieel cloudgebruik determination that triggered this exit-plan obligation, or satisfy clause 4.5(b)
    condition: "No prior /arckit-nl:nl-cloud assessment exists for this system"
  - command: risk
    description: Integrate residual exit risks (data-format lock-in, provider non-cooperation under disruption) into the risk register
    condition: "Residual risks identified in this plan are not yet reflected in ARC-*-RISK"
---
...
- **RBCLOUD** (Rijksbreed Cloudbeleid Compliance Assessment, from `/arckit-nl:nl-cloud`) — Extract: the materieel cloudgebruik determination that triggers this obligation, and whether this plan is being produced to satisfy clause 4.5(b) for an email/file-storage service
  - If missing: warn that the trigger for this exit plan has not been formally established, and recommend running `/arckit-nl:nl-cloud` first
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Next steps:
1. {If no prior /arckit-nl:nl-cloud assessment: Run /arckit-nl:nl-cloud to confirm the trigger for this plan}
2. {If clause 4.5(b) is the trigger: Update /arckit-nl:nl-cloud Section 3.2 with this plan's reference}
...
- **This plan does not itself determine cloud eligibility** — that is `/arckit-nl:nl-cloud`'s job. This command assumes a hosting decision already exists (or is being justified under clause 4.5(b)) and plans the exit from it.
...
```text
/arckit-nl:nl-exit Produce the exit plan for a departmental email and document collaboration service currently in public cloud, no prior exit test conducted

/arckit-nl:nl-exit Exit plan for 001, case management platform migrating to public cloud under a confirmed materieel cloudgebruik determination

/arckit-nl:nl-exit Annual review update to the exit plan for a shared-services platform, provider contract renewed, update the self-test evidence
```
