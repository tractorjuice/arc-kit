...
handoffs:
  - command: dpia
    description: Run a full Data Protection Impact Assessment if AT DSB screening flags high risk
    condition: "2+ AT DPIA criteria triggered or DSB published Blacklist applies"
  - command: eu-rgpd
    description: Run the pan-EU GDPR baseline first if not already completed
    condition: "No prior eu-rgpd assessment exists for this project"
  - command: at-nisg
    description: Assess NISG obligations where personal data is processed by Essential/Important entities
    condition: "Entity potentially qualifies as Essential or Important under NISG"
---
...
- **RGPD** (EU RGPD Assessment) — Extract: legal basis mapping, DPIA screening results, DPO determination, international transfer analysis
  - If missing: warn that `/arckit-at:at-dsgvo` should be run after `/arckit-eu:eu-rgpd` for best results. Proceed with available data.
...
- **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/at-dsgvo-template.md`
- **Then read** `${CLAUDE_PLUGIN_ROOT}/templates/_partials/RENDERING.md` and resolve the `<!-- DOC-CONTROL-HEADER -->` marker in the template before writing. `ATDSG` carries the Austria regime, which hard-routes to `_partials/document-control-at.md` (Offen / Eingeschränkt / Vertraulich / Geheim / Streng geheim) whatever the operator's user config says.
...
3. {If no eu-rgpd baseline: Run /arckit-eu:eu-rgpd first}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```text
/arckit-at:at-dsgvo Austrian DSG layer for 001 — federal ministry HR system with CCTV at entrances, employee data, and potential monitoring of IT usage

/arckit-at:at-dsgvo Assess AT DSG obligations for a Vienna regional hospital group integrating with ELGA, processing Gesundheitsdaten, planning mobile patient portal

/arckit-at:at-dsgvo AT data protection for a research consortium processing pseudonymised health data for a longitudinal cohort study under §§7–8 DSG
```
