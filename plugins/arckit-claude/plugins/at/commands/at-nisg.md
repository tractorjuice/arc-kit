...
- **NIS2** (EU NIS2 Assessment) — Extract: Annex I / Annex II classification, size threshold results, Article 21 ten-measure status, incident reporting baseline
  - If missing: warn that `/arckit-at:at-nisg` should be run after `/arckit-eu:eu-nis2` for best results
...
- **Then read** `${CLAUDE_PLUGIN_ROOT}/templates/_partials/RENDERING.md` and resolve the `<!-- DOC-CONTROL-HEADER -->` marker in the template before writing. `ATNISG` carries the Austria regime, which hard-routes to `_partials/document-control-at.md` (Offen / Eingeschränkt / Vertraulich / Geheim / Streng geheim) whatever the operator's user config says.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Next steps:
1. {If no eu-nis2 baseline: run /arckit-eu:eu-nis2 first}
2. {If personal data in security monitoring: run /arckit-at:at-dsgvo}
...
```text
/arckit-at:at-nisg Assess NISG obligations for a Styrian regional energy distributor (Stromnetzbetreiber) with BwD designation under NISG 2018, 400 employees, operating a SCADA migration project

/arckit-at:at-nisg NISG scoping for 001 — Austrian MSP serving healthcare and finance customers, 180 employees, HQ in Vienna with a secondary site in Linz

/arckit-at:at-nisg Austrian NIS2 transposition assessment for a federal ministry IT service provider, public administration sector, including CSIRT reporting readiness (NIS2-Meldeplattform)
```
