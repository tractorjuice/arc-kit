...
- **Manufacturing a source to fill the Source column.** `/arckit:wardley` writes a Component Inventory whose Source column is checked against the project before the file lands: an ARC document ID that does not exist, or a citation the map never declared, blocks the write. `Assumption` is always available and always acceptable — use it for components that are your own judgement, and record them in the Assumptions section. Inventing `ARC-001-RSCH-v1.0` because it looks plausible is the failure this catches.
- **Retyping a value chain's visibility instead of copying it.** Where a component's Source is the WVCH artefact, its visibility must be that artefact's number to 2dp — the value chain owns visibility, the map owns evolution. Re-deriving it from scratch produces two governance documents that quietly disagree. If the value chain is wrong, fix it with `/arckit:wardley.value-chain` and re-run; if the map genuinely re-anchors on a different user need, source those components elsewhere rather than to the value chain.
