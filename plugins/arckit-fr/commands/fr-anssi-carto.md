...
handoffs:
  - command: fr-ebios
    description: Use the cartography ecosystem map and attack surface summary as Workshop 3 input
    condition: "Cartography reveals interconnections and trust boundaries that need risk analysis"
  - command: fr-anssi
    description: Use cartography findings to prioritise ANSSI hygiene measures assessment
    condition: "Network view reveals exposed interfaces or unprotected sensitive flows"
  - command: diagram
    description: Generate architecture diagrams from the cartography data
    condition: "Visual diagram representation of cartography levels is needed"
  - command: secure
    description: Address security findings from the cartography attack surface analysis
    condition: "Cartography reveals unacceptable attack surface exposure"
---
...
- **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/fr-anssi-carto-template.md`
- **Then read** `${CLAUDE_PLUGIN_ROOT}/templates/_partials/RENDERING.md` and resolve the `<!-- DOC-CONTROL-HEADER -->` marker in the template before writing. `CARTO` carries the France regime, which hard-routes to `_partials/document-control-fr.md` whatever the operator's user config says.
...
Before writing the file, read `${CLAUDE_PLUGIN_ROOT}/references/quality-checklist.md` and verify all **Common Checks** plus **CARTO** per-type checks pass.
