...
Use the template at `${VIBE_EXTENSION_ROOT}/templates/sobc-template.md` as the structure. Fill in with:
...
Before writing the file, read `${VIBE_EXTENSION_ROOT}/references/quality-checklist.md` and verify all **Common Checks** plus the **SOBC** per-type checks pass. Fix any failures before proceeding.
