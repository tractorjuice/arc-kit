...
Use enum values from `${VIBE_EXTENSION_ROOT}/schemas/gov-reuse-handoff.schema.json` so the profile aligns with the reader's evidence allowlist.
...
- If matched: rubric = `${VIBE_EXTENSION_ROOT}/schemas/scoring-rubrics/gov-reuse-uk-gov.yaml`
- Otherwise: rubric = `${VIBE_EXTENSION_ROOT}/schemas/scoring-rubrics/gov-reuse-generic.yaml`
...
Ensure `${VIBE_EXTENSION_ROOT}/scripts/validate-handoff.mjs` exists via `Read`. If missing, stop and tell the user the plugin install is incomplete.
...
3. The reader's final-message string is a JSON payload. Write it to a tempfile via Bash, run the validator, and capture the result:
   ...
   node "${VIBE_EXTENSION_ROOT}/scripts/validate-handoff.mjs" \
        "${VIBE_EXTENSION_ROOT}/schemas/gov-reuse-handoff.schema.json" \
        "$TMPFILE"
   echo "exit=$?"
   rm -f "$TMPFILE"
   ```
...
- **Templates** — `${VIBE_EXTENSION_ROOT}/templates/gov-reuse-template.md` · `${VIBE_EXTENSION_ROOT}/templates/tech-note-template.md` (read by writer)
- **Schemas** — `${VIBE_EXTENSION_ROOT}/schemas/gov-reuse-handoff.schema.json` · `${VIBE_EXTENSION_ROOT}/schemas/scoring-rubrics/gov-reuse-{generic,uk-gov}.yaml`
