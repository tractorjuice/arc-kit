...
2. **Read the template** (with user override support):
   ...
   - **If not found**: Read `${VIBE_EXTENSION_ROOT}/templates/framework-overview-template.md` (default)
...
Before writing files, read `${VIBE_EXTENSION_ROOT}/references/quality-checklist.md` and verify all **Common Checks** plus the **FWRK** per-type checks pass. Fix any failures before proceeding.
