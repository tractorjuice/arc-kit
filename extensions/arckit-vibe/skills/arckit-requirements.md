...
1. **Identify the target project**:
   ...
   - Use Glob to list `projects/*/` directories and find the project matching the user's input (by name or number)
...
2. **Read existing artifacts**:
   ...
   **MANDATORY** (warn if missing):
   ...
   - **STKE** (Stakeholder Analysis) — Use `glob pattern="projects/**/ARC-*-STKE-*.md"` to find stakeholder analysis. Extract: goals, priorities, drivers, conflict analysis, RACI matrix
     - If missing: warn user to run `/arckit:stakeholders` first

   **RECOMMENDED** (read if available, note if missing):

   - **PRIN** (Architecture Principles, in 000-global) — Use `glob pattern="projects/000-global/ARC-000-PRIN-*.md"` to find principles. Extract: technology standards, constraints, compliance requirements for NFR alignment
     - If missing: suggest running `/arckit:principles` first
   - **RISK** (Risk Register) — Use `glob pattern="projects/**/ARC-*-RISK-*.md"` to find risk register. Extract: risk-driven requirements, mitigations that need NFRs
   - **SOBC** (Business Case) — Use `glob pattern="projects/**/ARC-*-SOBC-*.md"` to find business case. Extract: benefits, cost constraints, ROI targets for BR alignment

   **OPTIONAL** (read if available, skip silently):

   - **PLAN** (Project Plan) — Use `glob pattern="projects/**/ARC-*-PLAN-*.md"` to find plan. Extract: timeline constraints, phasing for requirement prioritization

3. **Read external documents and policies**:

   - Use `glob pattern="projects/**/external/**/*.{md,pdf,docx}"` to find external documents. Extract: requirements, constraints, scope definitions, acceptance criteria, legacy system interfaces
   - Use `glob pattern="projects/000-global/policies/*.{md,pdf,docx}"` to find global policies. Extract: mandatory compliance requirements, technology constraints, security standards
   - Use `glob pattern="projects/000-global/external/*.{md,pdf,docx}"` to find enterprise standards. Extract: cross-project requirements patterns
...
8. **Identify and resolve conflicting requirements**:
   ...
   - Use `glob pattern="projects/**/ARC-*-STKE-*.md"` and read to find the conflict analysis section
...
1. Use Glob to find existing `projects/{project-dir}/ARC-{PROJECT_ID}-REQ-v*.md` files
...
- Use Glob to find architecture principles
