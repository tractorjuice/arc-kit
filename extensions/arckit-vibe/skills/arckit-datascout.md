...
- If matched: rubric = `${VIBE_EXTENSION_ROOT}/schemas/scoring-rubrics/uk-gov.yaml`
- Otherwise: rubric = `${VIBE_EXTENSION_ROOT}/schemas/scoring-rubrics/generic.yaml`
...
3. The reader's final-message string is a JSON payload. Write it to a tempfile via Bash, run the validator, and capture the result. The validator's stdout is the normalised JSON on exit 0, or `{ok: false, errors: [{path, msg}]}` on exit non-zero:
   ...
   node "${VIBE_EXTENSION_ROOT}/scripts/validate-handoff.mjs" \
        "${VIBE_EXTENSION_ROOT}/schemas/datascout-handoff.schema.json" \
        "$TMPFILE"
   echo "exit=$?"
   rm -f "$TMPFILE"
   ```
...
- **Templates** — `${VIBE_EXTENSION_ROOT}/templates/datascout-template.md` (read by writer)
- **Schemas** — `${VIBE_EXTENSION_ROOT}/schemas/datascout-handoff.schema.json` · `${VIBE_EXTENSION_ROOT}/schemas/scoring-rubrics/{generic,uk-gov}.yaml`
