...
2. If `${args}` contains a bare project number (e.g. `001`) or name fragment, glob `projects/{NUMBER}-*/` or `projects/*-*{NAME}*/` and use the unique match. If multiple match, ask the user to disambiguate before proceeding — do not default to "most recent".
...
Use enum values from `${VIBE_EXTENSION_ROOT}/schemas/grants-handoff.schema.json` so the profile aligns with the reader's evidence allowlist.
...
- If matched: rubric = `${VIBE_EXTENSION_ROOT}/schemas/scoring-rubrics/grants-uk-gov.yaml`
- Otherwise: rubric = `${VIBE_EXTENSION_ROOT}/schemas/scoring-rubrics/grants-generic.yaml`
...
Ensure `${VIBE_EXTENSION_ROOT}/scripts/validate-handoff.mjs` exists via `Read`. The validator is pure Node with no npm dependencies, so its mere presence is sufficient. If missing, stop and tell the user the plugin install is incomplete.
...
3. The reader's final-message string is a JSON payload. Write it to a tempfile via Bash, run the validator, and capture the result. The validator's stdout is the normalised JSON on exit 0, or `{ok: false, errors: [{path, msg}]}` on exit non-zero:
   ...
   node "${VIBE_EXTENSION_ROOT}/scripts/validate-handoff.mjs" \
        "${VIBE_EXTENSION_ROOT}/schemas/grants-handoff.schema.json" \
        "$TMPFILE"
   echo "exit=$?"
   rm -f "$TMPFILE"
   ```
...
- **Validator script missing**: stop and tell the user the plugin install is incomplete (`${VIBE_EXTENSION_ROOT}/scripts/validate-handoff.mjs` should be present in any released plugin version ≥ 4.16.1). Do not silently fall back; the structural isolation guarantee depends on validation actually running.
...
- **Templates** — `${VIBE_EXTENSION_ROOT}/templates/grants-template.md` · `${VIBE_EXTENSION_ROOT}/templates/tech-note-template.md` (read by writer)
- **Schemas** — `${VIBE_EXTENSION_ROOT}/schemas/grants-handoff.schema.json` · `${VIBE_EXTENSION_ROOT}/schemas/scoring-rubrics/grants-{generic,uk-gov}.yaml`
- **Helpers** — `${VIBE_EXTENSION_ROOT}/scripts/validate-handoff.mjs` · `${VIBE_EXTENSION_ROOT}/scripts/bash/create-project.sh` · `${VIBE_EXTENSION_ROOT}/scripts/bash/generate-document-id.sh`
