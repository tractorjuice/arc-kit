...
display_name: ArcKit Gov Code Search
...
2. **Launch the agent**: Launch the **arckit-gov-code-search** agent in `acceptEdits` mode with the following prompt:
   ...
   ```text
   Search UK government repositories for: {${args}}
...
2. **Read the template** (with user override support):
   ...
   - **If not found**: Read `${VIBE_EXTENSION_ROOT}/templates/gov-code-search-template.md` (default)
