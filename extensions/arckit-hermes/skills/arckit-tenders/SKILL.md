...
Triggers: "/arckit:tenders", "arckit-tenders", "Procurement market intelligence — award-value benchmarks, to".
...
1. If the user's `{topic}` contains an explicit `projects/{NNN}-{name}/` path, use that path verbatim.
2. If `{topic}` contains a bare project number (e.g. `002`) or name fragment, glob `projects/{NUMBER}-*/` or `projects/*-*{NAME}*/` and use the unique match. If multiple match, ask the user to disambiguate before proceeding — do not default to "most recent".
...
neither file is present, proceed using the explicit scope in `{topic}`
...
From `{topic}`, after stripping the project hint:
...
- **No requirements**: not a failure here. Proceed with the explicit
  `{topic}` scope and say so. (`/arckit:datascout` requires requirements;
