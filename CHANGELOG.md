...
- **`arckit-uk-teal` community plugin — UK Government Teal Book project-delivery overlay (12th marketplace plugin).** Adds a UK Government domain overlay aligned to [The Teal Book](https://projectdelivery.gov.uk/teal-book/home/) (NISTA / Government Project Delivery) and the [GovS 002 Project Delivery Functional Standard](https://www.gov.uk/government/publications/project-delivery-functional-standard). The Teal Book is the "how" companion to the "what" of GovS 002 and sits alongside HM Treasury's Green Book; ArcKit core already covers the Green Book business case (`/arckit:sobc`), the GDS delivery plan (`/arckit:plan`), and the Orange Book risk register (`/arckit:risk`), so the overlay adds only the three Teal-specific delivery-governance artefacts core lacks and cross-references the rest. **3 commands:** `uk-teal-ciaf` (GovS 002 Continuous Improvement Assessment Framework capability self-assessment — themed, four-stage maturity, doc-type `TEALCIAF`), `uk-teal-tailoring` (Delivery Management Approach tailoring the Teal Book practices, Part B / Chapter 9, mapped to GovS 002 mandatory requirements, doc-type `TEALDMA`), and `uk-teal-assurance` (Integrated Assurance and Approval Plan with IPA/NISTA gateway-review readiness across the three lines of defence, doc-type `TEALIAAP`). The three new commands handoff into the core delivery artefacts (`sobc`, `plan`, `risk`, `conformance`, `maturity-model`) — the cross-reference flows overlay→core, preserving the invariant that core never depends on an optional overlay.
- **`uk-teal-delivery` build recipe** (`arckit-claude/skills/arckit-build/recipes/`) — composes Teal Book delivery governance on the UK Government baseline (principles, requirements, stakeholders, Green Book SOBC, GDS plan, Orange Book risk, roadmap, traceability) plus the three Teal artefacts.
- **3 doc-type codes** registered in `arckit-claude/config/doc-types.mjs` (and the `/arckit:pages` allow-list): `TEALCIAF`, `TEALDMA`, `TEALIAAP` (regime `UK`, HIGH severity). Templates mirrored into `.arckit/templates/` (CLI package data) and propagated to all five non-Claude extensions via `scripts/converter.py` (`PLUGIN_SOURCES` += `arckit-uk-teal`).
- **Status:** Community-contributed, **EXPERIMENTAL** — recruiting a domain co-maintainer. The Teal Book V1 is published for a trial period running to **31 December 2026**; output is not assurance, commercial, or regulatory advice and does not constitute an IPA/NISTA assurance review.
