...
- **OpenCode support in `arckit` CLI** — `arckit init --ai opencode`
  - Initializes project with OpenCode configuration (`.opencode/opencode.json`), commands (`.opencode/commands/`), and agents (`.opencode/agents/`) generated by `scripts/converter.py`
  - Extension manifest (`opencode.json`) with MCP servers (AWS Knowledge, Microsoft Learn, Google Developer Knowledge via mcp-remote) and optional Google API Key setting
  - All 48 commands and 5 agents
  - Sets up `.envrc` for OpenCode environment
