...
- **First**, check if `.arckit/templates/uk-mod-secure-template.md` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/uk-mod-secure-template.md` (default)
...
- `[COMMAND]` → "arckit-uk-mod:uk-mod-secure"
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/arckit-uk-mod:uk-mod-secure` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/arckit-uk-mod:uk-mod-secure` command
...
Before writing the file, read `${CLAUDE_PLUGIN_ROOT}/references/quality-checklist.md` and verify all **Common Checks** plus the **SECD-MOD** per-type checks pass. Fix any failures before proceeding.
