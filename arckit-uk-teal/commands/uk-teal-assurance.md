...
argument-hint: "<project ID or initiative, e.g. '001', 'major programme assurance plan'>"
effort: high
keep-coding-instructions: true
handoffs:
  - command: risk
    description: Assurance activities should target the highest Orange Book risks — cross-reference the project risk register so reviews focus on what matters.
  - command: sobc
    description: The IAAP schedules assurance around the business-case approval points (SOBC/OBC/FBC) — run sobc first if the business case is not present.
  - command: plan
    description: Align the planned assurance reviews to the project plan's stage gates and milestones.
  - command: conformance
    description: The architecture conformance assessment is a source of technical-assurance evidence feeding the IAAP schedule.
  - command: uk-teal-ciaf
    description: Assess the organisation's underlying assurance capability and integrated assurance framework maturity.
---
...
3. **Read the template and partials** (check `.arckit/templates-custom/` first, then `.arckit/templates/`, then fall back to the plugin copy):

   - `.arckit/templates-custom/uk-teal-assurance-template.md` → `.arckit/templates/uk-teal-assurance-template.md` → `${CLAUDE_PLUGIN_ROOT}/templates/uk-teal-assurance-template.md`
   - `${CLAUDE_PLUGIN_ROOT}/templates/_partials/RENDERING.md` — resolves the `<!-- DOC-CONTROL-HEADER -->` marker and the Classification substitution
   - `${CLAUDE_PLUGIN_ROOT}/references/citation-instructions.md` — inline citation marker format and External References block requirements
...
8. **Populate the External References section** per `${CLAUDE_PLUGIN_ROOT}/references/citation-instructions.md`. The Teal Book, GovS 002, the IPA/NISTA assurance-and-approvals collection, the Green Book, and NISTA MUST appear in the Document Register, each with a "verify against source" note where the URL could not be fetched.
