...
argument-hint: "<project ID or initiative, e.g. '001', 'digital case-management programme'>"
effort: high
keep-coding-instructions: true
handoffs:
  - command: plan
    description: Turn the tailored delivery approach into a phased project plan with stages and decision gates.
  - command: sobc
    description: The business-case approval points (Green Book five-case) referenced in this approach are produced by the SOBC command.
  - command: uk-teal-assurance
    description: Build the integrated assurance and approval plan that matches the tailoring and gates defined here.
  - command: risk
    description: Size an Orange Book risk register proportionate to the tailoring decisions and risk tier recorded here.
  - command: uk-teal-ciaf
    description: Assess the organisational capability to deliver this tailored approach (Capability in Assurance Framework).
---
...
3. **Read the template** (check overrides first, fall back in order):
   ...
   - `${CLAUDE_PLUGIN_ROOT}/templates/uk-teal-tailoring-template.md`
   ...
   - `${CLAUDE_PLUGIN_ROOT}/templates/_partials/RENDERING.md` — resolves the `<!-- DOC-CONTROL-HEADER -->` marker and Classification substitution.
   - `${CLAUDE_PLUGIN_ROOT}/references/citation-instructions.md` — inline citation marker format and External References requirements.
