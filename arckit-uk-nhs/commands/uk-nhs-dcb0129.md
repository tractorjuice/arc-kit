...
handoffs:
  - command: uk-nhs-dcb0160
    description: Deployer-side companion. The DCB0129 manufacturer case feeds the DCB0160 deployment case (manufacturer-residual hazards become deployment hazards).
  - command: uk-nhs-dtac
    description: DTAC Section 1 (Clinical Safety) consumes the SAFETY-CASE.md and HAZARD-LOG.md outputs.
  - command: uk-mdr-classification
    description: If the product is a medical device, DCB0129 hazards inform ISO 14971 risk file and classification rationale.
  - command: risk
    description: Project-level risk register cross-references hazards by H-ID.
  - command: dpia
    description: Privacy hazards in HAZARD-LOG.md cross-reference DPIA risks where they overlap.
---
...
2. **Read the templates** (four files):
   ...
   - **Fallback**, `${CLAUDE_PLUGIN_ROOT}/templates/uk-nhs-dcb0129-*-template.md`
...
- **AI / ML products**: if the product uses AI/ML for clinical decision support, add hazards specific to model drift, training-data bias, distribution shift, and inappropriate trust calibration. Cross-reference `/arckit-uk:uk-atrs` output if present.
