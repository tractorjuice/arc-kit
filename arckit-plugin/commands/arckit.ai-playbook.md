...
3. **Read Available Documents**:
   ...
   **Read the template** (with user override support):
   ...
   - **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/uk-gov-ai-playbook-template.md` (plugin default)

   > **Note**: Read the `VERSION` file and update the version in the template metadata line when generating.
   > **Tip**: Users can customize templates with `/arckit.customize uk-gov-ai-playbook`
...
DOC_ID=$(.arckit/scripts/bash/generate-document-id.sh "${PROJECT_ID}" "AIPB" "${VERSION}")
