...
4. **Read the template** (with user override support):
   ...
   - **If not found**: Read `${CLAUDE_PLUGIN_ROOT}/templates/requirements-template.md` (default)
   - **Update Template Version**: Read the `${CLAUDE_PLUGIN_ROOT}/VERSION` file and replace the version number in the template metadata line:
     ```
     > **Template Status**: [keep] | **Version**: [from ${CLAUDE_PLUGIN_ROOT}/VERSION] | **Command**: [keep]
     ```
...
DOC_ID=$(${CLAUDE_PLUGIN_ROOT}/scripts/bash/generate-document-id.sh "${PROJECT_ID}" "REQ" "${VERSION}")
