...
Background monitors declared via the `experimental.monitors` key in `plugin.json` (v2.1.105+, moved under `experimental` in v2.1.129). Each entry runs as a persistent subprocess; stdout lines are delivered as session notifications. Schema: `name`, `command` (supports `${CLAUDE_PLUGIN_ROOT}` and `${user_config.*}`), `description`, `when` (`always` or `on-skill-invoke:<skill>`). **Needs v2.1.144** — earlier versions used monitor stdout to title new sessions, so the `stale-artifact-scan` description leaked into session names instead of the user's first prompt. (ArcKit's overall min-version floor is **v2.1.219**, set in `version-check.mjs` — the release that fixes project-scoped plugin loading from git worktrees and `claude agents --plugin-dir` plugin agent/skill visibility, while carrying the v2.1.198+ background-subagent reliability fixes and the v2.1.172 wildcard-domain `WebFetch` fix.)
