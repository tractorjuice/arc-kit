...
1. **CLI package** (`src/arckit_cli/`) -- Python CLI installed via `pip`/`uv`, runs `arckit init` to scaffold projects for **Codex CLI** (copies templates/commands into them)
...
# Convert Claude commands to Codex and Gemini extension formats
...
**Multi-AI note**: Agents are Claude Code only. Codex and Gemini extension equivalents keep the full inline prompts since they don't support the Task/agent architecture. The converter (`scripts/converter.py`) automatically extracts agent prompts and inlines them into Codex Markdown and Gemini extension TOML formats.
...
**Init flags:**
- `--ai codex` - Select AI assistant. `--ai claude` redirects to plugin installation. `--ai gemini` redirects to extension installation.
...
5. Run `python scripts/converter.py` to generate Codex Markdown (`.codex/`) and Gemini extension TOML (`arckit-gemini/`)
6. Test plugin: Open a test repo with the plugin enabled and run the command
7. Test CLI: `arckit init test --ai codex --no-git && cd test && codex`
