...
**`scripts/converter.py`**: Config-driven converter using `AGENT_CONFIG` dictionary — adding a new AI target only requires a new dict entry. Converts plugin commands (`arckit-plugin/commands/*.md`) to all configured output formats (currently: Codex Markdown `.codex/`, OpenCode Markdown `.opencode/` + `arckit-opencode/`, Gemini extension TOML `arckit-gemini/`). Rewrites `${CLAUDE_PLUGIN_ROOT}` paths, copies supporting files to extension dirs, and extracts full agent prompts for non-Claude targets. Key functions: `rewrite_paths(prompt, config)`, `format_output(description, prompt, fmt)`, `convert(commands_dir, agents_dir)`, `copy_extension_files(plugin_dir)` — all driven by `AGENT_CONFIG`
