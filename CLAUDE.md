...
PostToolUse hook on `Write|Edit` against `projects/**` that injects a `## Provenance` block at the end of every ArcKit artefact (`ARC-NNN-*-vN.N.md`). Block is delimited by `<!-- arckit-provenance:start -->` / `<!-- arckit-provenance:end -->` markers and rewritten in place on subsequent runs (idempotent).

Captured fields:

- **Skill** — `target.skill` from `.arckit/state.json` if the file is a build-harness target; otherwise `arckit:` + the command parsed from the existing `Generated by:` footer line
- **Build context** (recipe / wave / target / topic) — only present when run via `/arckit:build`, sourced from `state.json`
- **Requested Effort** — `effort:` field from the invoking command's YAML frontmatter
- **Effective Effort** — derived via the silent-downgrade matrix in `provenance-stamp.mjs` (`MODEL_MAX_EFFORT`); placeholder until Claude Code surfaces the active model to hooks (see #407 / claude-code#55829)
- **Model** — placeholder for the same reason; the hook is wired to populate this once `transcript_path` parsing or an env var is added
- **ArcKit Version** — `arckit-claude/VERSION`
- **Generated on** — ISO 8601 timestamp (per write)

Existing artefacts pre-dating the hook are stamped on the next Write/Edit. The block is markdown-rendered and visible to human reviewers (UK Gov audit requirement).
...
The standard footer is human-authored by the command. A second, machine-stamped block follows it whenever the artefact lives under `projects/**` — see §"Provenance stamping" under "Plugin Hooks". Templates do not need to include the provenance block; it is appended automatically by `provenance-stamp.mjs`.
