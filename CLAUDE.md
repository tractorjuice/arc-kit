...
The CLI and plugin have independent version numbers (CLI: `VERSION` + `pyproject.toml`, Plugin: `arckit-plugin/VERSION` + `arckit-plugin/.claude-plugin/plugin.json`). Claude Code support was removed from the CLI in favour of the plugin, which references files in-place via `${CLAUDE_PLUGIN_ROOT}`.
...
- `.gemini/commands/arckit/*.toml` - Gemini commands (generated by `scripts/converter.py`)
- `.codex/prompts/arckit.*.md` - Codex prompts
...
**Gemini CLI**: `.gemini/commands/arckit/{name}.toml` - TOML format (generated by `scripts/converter.py`)
**Codex CLI**: `.codex/prompts/arckit.{name}.md` - Markdown format
...
**`scripts/converter.py`**: Converts plugin commands (`arckit-plugin/commands/*.md`) to Gemini format (`.gemini/commands/arckit/*.toml`) and Codex Markdown (`.codex/prompts/arckit.*.md`). For agent-delegating commands, extracts the full agent prompt (not the thin wrapper) since Gemini/Codex don't support the Task/agent architecture
...
5. Run `python scripts/converter.py` to generate Gemini TOML (`.gemini/`) and Codex Markdown (`.codex/`)
