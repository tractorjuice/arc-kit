...
ArcKit is an **Enterprise Architecture Governance & Vendor Procurement Toolkit** providing 48 slash commands for AI coding assistants (Claude Code, Codex CLI, Gemini CLI) to generate architecture artifacts. It transforms architecture governance from scattered documents into a systematic, template-driven process.

**Three distribution formats** exist side-by-side in this repo:
...
The CLI and plugin have independent version numbers (CLI: `VERSION` + `pyproject.toml`, Plugin: `arckit-plugin/VERSION` + `arckit-plugin/.claude-plugin/plugin.json`). The Gemini extension version tracks the plugin version (`arckit-gemini/VERSION` + `arckit-gemini/gemini-extension.json`). Claude Code support was removed from the CLI in favour of the plugin, which references files in-place via `${CLAUDE_PLUGIN_ROOT}`. The Gemini extension is generated by `scripts/converter.py` which rewrites paths and copies supporting files.
...
arckit init test-project --ai gemini --no-git
arckit init test-project --all-ai --no-git    # Install all AI formats (Gemini + Codex)
arckit init test-project --ai codex --minimal  # Skip docs/guides
cd test-project && ls -la .gemini/commands/

# Build distribution
python -m build

# Convert Claude commands to Gemini CLI, Codex, and Gemini extension formats
...
- `.gemini/commands/arckit/*.toml` - Gemini CLI commands (generated by `scripts/converter.py`, paths -> `.arckit`)
- `.codex/prompts/arckit.*.md` - Codex prompts (generated by `scripts/converter.py`, paths -> `.arckit`)
- `arckit-gemini/commands/arckit/*.toml` - Gemini extension commands (generated, paths -> `~/.gemini/extensions/arckit`)
...
**Gemini CLI**: `.gemini/commands/arckit/{name}.toml` - TOML format (generated by `scripts/converter.py`, paths rewritten to `.arckit`)
**Gemini Extension**: `arckit-gemini/commands/arckit/{name}.toml` - TOML format (generated, paths rewritten to `~/.gemini/extensions/arckit`)
**Codex CLI**: `.codex/prompts/arckit.{name}.md` - Markdown format (generated, paths rewritten to `.arckit`)
...
├── .gemini/commands/     # Gemini commands (or .codex/prompts/, or both with --all-ai)
...
**Init flags:**
- `--ai gemini|codex` - Select AI assistant (default: prompt). `--ai claude` redirects to plugin installation.
- `--all-ai` - Install commands for all CLI-supported AI assistants (Gemini + Codex)
...
**`scripts/converter.py`**: Converts plugin commands (`arckit-plugin/commands/*.md`) to three output formats: Gemini CLI TOML (`.gemini/`), Codex Markdown (`.codex/`), and Gemini extension TOML (`arckit-gemini/`). Rewrites `${CLAUDE_PLUGIN_ROOT}` paths (CLI: `.arckit`, extension: `~/.gemini/extensions/arckit`). Also copies templates, scripts, guides, and skills to the extension directory. For agent-delegating commands, extracts the full agent prompt (not the thin wrapper) since Gemini/Codex don't support the Task/agent architecture
...
5. Run `python scripts/converter.py` to generate Gemini CLI TOML (`.gemini/`), Codex Markdown (`.codex/`), and Gemini extension TOML (`arckit-gemini/`)
6. Test plugin: Open a test repo with the plugin enabled and run the command
7. Test CLI: `arckit init test --ai gemini --no-git && cd test && gemini`
