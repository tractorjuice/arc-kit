...
export function runSyncGuidesHook(data = {}) {

// Guard: accept raw Claude-style slash command, raw Codex skill invocation,
...
const pluginRoot = process.env.CODEX_PLUGIN_ROOT || process.env.CLAUDE_PLUGIN_ROOT || resolve(__dirname_hook, '..');
const sourceDir = join(pluginRoot, 'docs', 'guides');
if (!isDir(sourceDir)) return null;
...
const customTemplatePath = join(repoRoot, '.arckit', 'templates-custom', 'pages-template.html');
const legacyCustomTemplatePath = join(repoRoot, '.arckit', 'templates', 'pages-template.html');
...
} else if (isFile(legacyCustomTemplatePath)) {
  templatePath = legacyCustomTemplatePath;
  templateSource = 'legacy custom override';
...
function main() {
  const output = runSyncGuidesHook(parseHookInput());
