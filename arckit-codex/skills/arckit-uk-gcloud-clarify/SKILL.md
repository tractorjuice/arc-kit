...
After using `/arckit-uk:uk-gcloud-search` to find G-Cloud services, you have a shortlist but face challenges:
...
- **GCLD** (G-Cloud Search, in procurement/) — Extract: Shortlisted services (top 3-5), service names, supplier names, service links, key features, Must-Have Match scores, Desirable Features scores, compliance mentions, pricing
  - If missing: ERROR "Run `/arckit-uk:uk-gcloud-search` first — need service search results"
...
- `[COMMAND]` → "arckit-uk:uk-gcloud-clarify"
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/arckit-uk:uk-gcloud-clarify` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/arckit-uk:uk-gcloud-clarify` command
...
**Generated**: [DATE]
**Tool**: /arckit-uk:uk-gcloud-clarify
...
- **No gcloud-ARC-*-REQ-*.md**: ERROR "G-Cloud search results not found - run /arckit-uk:uk-gcloud-search first"
...
2. `/arckit-uk:uk-gcloud-search` → Find services on Digital Marketplace
3. **`/arckit-uk:uk-gcloud-clarify`** → Identify gaps, generate questions
