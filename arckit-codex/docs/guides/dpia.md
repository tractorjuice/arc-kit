...
`/arckit-uk:uk-dpia` delivers a GDPR-compliant DPIA whenever processing could present high risk to individuals.
...
```bash
/arckit-uk:uk-dpia Generate DPIA for <project>
```
...
- Reference DPIA outcomes in `/arckit-uk:uk-ai-playbook` and `/arckit-uk:uk-secure`.
