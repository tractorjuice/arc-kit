...
All 12 plugins come from the same `tractorjuice/arc-kit` marketplace. The 11 community plugins (`arckit-uae`, `arckit-fr`, `arckit-ca`, `arckit-eu`, `arckit-at`, `arckit-au`, `arckit-au-energy`, `arckit-us`, `arckit-uk-finance`, `arckit-uk-nhs`, `arckit-uk-teal`) require the `arckit` core plugin. `arckit-au-energy` (sector) additionally requires `arckit-au` (jurisdiction), which it composes — install with `claude plugin install arckit arckit-au arckit-au-energy`.
...
- Community overlays (`arckit-uae`, `arckit-fr`, `arckit-ca`, `arckit-eu`, `arckit-at`, `arckit-au`, `arckit-au-energy`, `arckit-us`, `arckit-uk-finance`, `arckit-uk-nhs`, `arckit-uk-teal`) are independent plugins — install only the jurisdictions / sectors you need. Each adds its own always-on baseline. `arckit-uk-finance`, `arckit-uk-nhs`, and `arckit-au-energy` are **sector** overlays (`arckit-au-energy` layers the energy sector on the `arckit-au` jurisdiction baseline); `arckit-uk-teal` is a UK Government **project-delivery methodology** overlay (Teal Book / GovS 002); the rest are jurisdiction-based.
...
## UK Government Teal Book Overlay (Community-contributed)

> ⚠️ **Community-contributed overlay — EXPERIMENTAL.** The 3 commands below cover UK Government **project-delivery governance** aligned to [The Teal Book](https://projectdelivery.gov.uk/teal-book/home/) (NISTA / Government Project Delivery) and the [GovS 002 Project Delivery Functional Standard](https://www.gov.uk/government/publications/project-delivery-functional-standard). The Teal Book is the practical "how" companion to the "what" of GovS 002 and is used alongside HM Treasury's Green Book. Teal Book V1 is published for a trial period running to **31 December 2026** — verify guidance against the live source before reliance. Output is **not** assurance, commercial, or regulatory advice and does not constitute an IPA/NISTA assurance review — it MUST be reviewed by the SRO and the organisation's Project Delivery / assurance function.

- `/arckit.uk-teal-ciaf` — GovS 002 **Continuous Improvement Assessment Framework (CIAF)** project-delivery capability self-assessment: theme-by-theme scoring on a four-stage maturity scale, a capability heat-map, and a prioritised continuous-improvement roadmap (`TEALCIAF`)
- `/arckit.uk-teal-tailoring` — **Delivery Management Approach** tailoring the Teal Book practices (Part B / Chapter 9) to a portfolio, programme, or project's size, complexity, risk, and setting, mapped to the mandatory requirements of GovS 002 (`TEALDMA`)
- `/arckit.uk-teal-assurance` — **Integrated Assurance and Approval Plan (IAAP)** mapping business-case approval points (SOBC → OBC → FBC) to a proportionate, coordinated assurance schedule with IPA/NISTA gateway-review readiness across the three lines of defence (`TEALIAAP`)

The overlay does **not** duplicate core — it adds the Teal-specific artefacts core lacks and cross-references the rest: the Green Book business case (`/arckit:sobc`), the GDS delivery plan (`/arckit:plan`), the Orange Book risk register (`/arckit:risk`), the strategic roadmap (`/arckit:roadmap`), architecture conformance as technical-assurance evidence (`/arckit:conformance`), and generic capability maturity (`/arckit:maturity-model`). Recipe: `uk-teal-delivery`. Install: `claude plugin install arckit arckit-uk-teal`. Help wanted: recruiting a UK government project-delivery domain co-maintainer — open an issue tagged `co-maintainer: uk-teal`.
